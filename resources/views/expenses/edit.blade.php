            ...
            <form action="{{ route('expenses.update', $expense) }}" method="POST">
                                ...
                                @if($type->isParent())
                                    <optgroup label="{{ $type->name }}">
                                        @foreach($type->children as $child)
                                            <option value="{{ $child->id }}" {{ old('expense_type_id', $expense->expense_type_id) == $child->id ? 'selected' : '' }}>
                                                {{ $child->name }}
                                            </option>
                                        @endforeach
                                    </optgroup>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" 
                              ...
                              rows="3"
                              class="form-textarea"
                              placeholder="أدخل وصف المصروف (اختياري)">{{ old('description', $expense->description) }}</textarea>
                    @error('description')
                        <p class="error-message">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="notes" 
                              ...
                              rows="3"
                              class="form-textarea"
                              placeholder="أي ملاحظات إضافية (اختياري)">{{ old('notes', $expense->notes) }}</textarea>
                    @error('notes')
                        <p class="error-message">{{ $message }}</p>
                    @enderror
                </div>
